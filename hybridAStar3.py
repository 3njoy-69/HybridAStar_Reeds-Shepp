...
import json
import cv2
from taG7 import get_car_position
...
def main():
    # Import the car position detection function
    from taG7 import get_car_position
    # Get car's initial position and orientation
    videoPath = ('/Users/laptopjp/Desktop/VS_Code/HybridAStar/output5.mp4')
    x, y, yaw = get_car_position(videoPath)
    # Set start position to detected car position
    s = [x*100, y*100, yaw]
    print(f"Start position s: {s}")
    # Set Goal position (you can modify this as needed)
    # s = [150, 150, np.deg2rad(0)]

    g = [400, 50, np.deg2rad(0)]
    print(f"Goal position g: {g}")   # ← In ra goal
